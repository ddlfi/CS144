...
struct PeekOnce : public Peek
{
  ...
  void execute( ByteStream& bs ) const override
  {
    cout<<"byte_stream_test_harness_hh_107"<<endl;
    auto peeked = bs.reader().peek();
    cout<<"byte_stream_test_harness_hh_109"<<endl;
    cout<<typeid(peeked).name()<<' '<<typeid(output_).name()<<endl;
    cout<<output_<<endl;
    cout<<peeked<<endl;
    if ( peeked != output_ ) {
      cout<<"byte_stream_test_harness_hh_112"<<endl;
      throw ExpectationViolation { "Expected exactly \"" + Printer::prettify( output_ ) + "\" at front of stream, "
                                   + "but found \"" + Printer::prettify( peeked ) + "\"" };
      cout<<"byte_stream_test_harness_hh_115"<<endl;
    }
    cout<<"byte_stream_test_harness_hh_117"<<endl;
  }
};
